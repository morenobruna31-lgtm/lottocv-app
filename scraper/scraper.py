...
def fetch_json(url, retries=3):
    for i in range(1, retries+1):
        try:
            ...
            log.warning(f"Tentativa {i}/{retries} falhou: {e}")
...
def scrape_ultimo_sorteio():
    ...
    if data:
        log.info(f"[API] Ultimo sorteio: {data.get('totoloto',{}).get('drawCode','?')}")
    return data

def scrape_jackpot_atual():
    data = fetch_json(f"{BASE_URL}/api/games?context=currentDraw")
    if data:
        log.info("[API] Jackpot obtido via currentDraw")
        return data
    return None
...
def extrair_jackpot(data):
    ...
    for jogo in ["totoloto", "joker"]:
        ...
        try:
            ...
            log.info(f"[Jackpot] {jogo}: {resultado[jogo]:,.0f} ECV")
...
def executar_scraping():
    ...
    try:
        ...
        if raw:
            ...
            jp = extrair_jackpot(jp_data) if jp_data else {"totoloto":0.0,"joker":0.0,"concurso_totoloto":"","data_totoloto":"","concurso_joker":"","data_joker":""}
            ...
            atualizar_jackpot("totoloto", jp["totoloto"],
                jp.get("concurso_totoloto") or t_info.get("drawCode",""),
                jp.get("data_totoloto") or t_info.get("drawDate","")[:10])
            atualizar_jackpot("joker", jp["joker"],
                jp.get("concurso_joker") or j_info.get("drawCode",""),
                jp.get("data_joker") or j_info.get("drawDate","")[:10])
